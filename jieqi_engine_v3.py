"""
揭棋引擎 v3 - jieqi_engine (v5.6) 的真静态搜索优化版 (其余逻辑完全一致):
  v5.6 基线: TT不清空 + 双时限 + 深度封顶 12。
  v3 优化: 真(递归)静态搜索。原先 depth==0 叶节点只挑一步最优吃子着法
      作为一次性加分 (不考虑对方回应, 也不递归后续互吃), 在吃子密集局面
      系统性高估吃子价值; 现改为标准 negamax 递归静态搜索: stand-pat 剪枝
      + MVV-LVA 排序的吃子着法递归 (层数封顶 qs_depth), 叶节点静态分
      = pos.score + 空头炮分差, 与原叶节点口径一致。
"""
...
class Searcher:
    ...
    def _static(self, pos):
        """[v3] 叶节点静态分 (行棋方视角), 与原 depth==0 口径一致。"""
        return pos.score + pos.kongtou_score - pos.kongtou_score_opponent

    def qsearch(self, pos, alpha, beta, qdepth):
        ...
        best = stand
        for m in caps:
            val = -self.qsearch(pos.move(m), -beta, -alpha, qdepth - 1)
            if val > best:
                best = val
                if val > alpha:
                    alpha = val
    ...
    def alphabeta(self, pos, alpha, beta, depth, root=True, nullmove=False, nullmove_now=False):
        ...
        if depth == 0:
            # [v3] 真递归静态搜索 (替代原"单步最优吃子一次性加分"的假静态搜索),
            #      在走法排序之前接入: 静态搜索自带 MVV-LVA 排序, 不依赖外层排序结果
            return self.qsearch(pos, alpha, beta, self.qs_depth)
