...
## 八、长将规则简化（v5.9）：纯配额制 + 记忆链 3/4 格校验

### 8.1 规则简化（键表制 → 纯配额制）

用户决策：键表制环检测成本过高，长将检测只保留连将次数，且比亚洲规则（6×子数）更严：
**配额 = min(3×将军子数, 9)**——单子 3 次、二子 6 次、三子及以上 9 次；
非将军着法整体重置计数（一将一闲永远不定罪，方案 A）；吃子不重置，被吃子配额不缩。
第三次重复判和保留作兜底。

删除：裁判 check_keys/adjudicate_long_check/full_history 与主循环键表分支；
引擎 _check_key_table/_key_blocks/_memory_blocks/_flip_board；协议停发全量 pos_history
（client/server 本就容错，零改动）。保留：PerpCheckTracker 认子机制、引擎配额拦截
（裁判模式 _quota_blocks / 实战模式 _memory_quota_blocks，公式同步 min(3n,9)）、
兜底着（depth=-2）。

实战模式判定从"不传 history"改为"不下发 check_state"（live = check_state is None），
否则裁判停发 history 后引擎会误把裁判对局当实战。引擎在实战下完全自主判断长将，
不依赖任何下发数据（修复过 v5.8 "安检挂在 pos_history 存在性上导致实战从不触发"的同类隐患）。

### 8.2 记忆链 diff 校验升级：4 格 → 3/4 格完整枚举

_memory_step 原只接受 4 格变化，对方落点与我方格重合时（占我方刚空的 src / 反吃我方落点）
退化为 3 格导致误断链。升级为完整枚举：4 格 rest 一空一占；3 格时 rest 必为空（对方起点），
落点 = estr[src] 非空 ? src : dst；其余形态照旧断链 fail-open。
代码内留 TODO：精细判断后续用走子标记视觉识别（直接识别对方走了哪个子）替换/完善。

### 8.3 验证

单测 14 项：配额边界（单子 3/4、二子 6/7、三子 9、四子封顶 9）、闲着重置、被吃不缩、
裁判全链路（单子 4 连将第 7 着判负 / 一将一闲不定罪三次重复判和）、
引擎两模式拦截（count=2 放行 / count=3 拦且兜底不再将军；实战前 3 将放行 / 第 4 将拦）。
diff 升级补测 5 项：4 格回归、3 格 A/B 延续、5 格异常与无起点形态仍断链。

### 8.4 等墙钟 15 局 vs v5.7（各 1.5s/着，种子 20260965~79）

新版 7 胜 5 负 3 和（首轮 10 局 6-3-1 + 补赛 5 局 1-2-2），用时对称（1.10~1.13 vs 1.09~1.14s/着）。
规则生效：配额判负 2 次全落 v5.7（971/973，连将 4 次超 min(3×1,9)=3），
三次重复判和 3 次（969/975/977）；新版零规则判负、零兜底着（搜索自身无凑 4 连将倾向，
拦截作保险存在）。剔除规则胜后战术层 5-5，符合"仅改规则层未动搜索评估"的预期。
记录：tools/games/q3_g1~10.log、q3b_g1~5.log 及对应 JSON/txt。
...
search() 原先丢弃每层迭代的根值 (alphabeta 返回值), 用 pos.value(move) 单步静态增量上报,
导致日志/棋谱分数与搜索结论严重脱节 (必死局面根值每层 -3696, 记录却只有 +2)。
修复: 接住根值作为 best_score (正常迭代与首层保底两处)。口径: 正=行棋方优, 杀王分 ±3696;
安检兜底着 (depth=-2) 仍报单步分 (无现成根值, 实战未触发过)。验证: 必死局面实测报 -3696.0。

### 9.2 手动截图 (main.py)

除引擎无着法时的自动识别调试图外, 新增守护线程热键手动截图: Windows 按 s 键 / macOS 按回车,
存 snapshot/manual_full_<ts>.png + manual_board_<ts>.png (原图, 不跑识别, 供人工核对/离线重识别)。

### 9.3 待查: 实战阵营误认事故

实战出现黑车 (7,4) 被认成红车, 导致被贴身将不应 (对局负)。疑点: 車/馬/炮为共用字,
阵营靠 _detect_side 颜色判定, 而阈值不对称 (red_ratio>0.05 先判红 vs dark_ratio>0.2 判黑),
疑被走子标记圈等偏红像素污染。待用户提供事故帧截图后定位参数。

---

## 十、v5.11 (Plan A) 10 局等墙钟复盘与因果归因 (2026-08-24)

### 10.0 结论速览

pypy(v5.11) 6W-1L-3D vs pypy510(v5.10 基线)，等墙钟 (双方 1.13s/着)、深度基线还略高
(6.10 vs 5.95)。净胜 5 局归因于 Plan A (TT 杀王分截断豁免)，证据链四层:
① 6 胜全部是败方引擎自己逐着确认 -3696 的客观强制杀; ② 杀王宣告转化率不对称
(v5.11 6/6 vs 基线 1/3); ③ g5 第 46 着现场复现——同一局面基线 2/3 次染出假杀王 +3696,
修复版 3/3 次稳定 +224 从不宣告杀王; ④ g6 的假宣告经复现排除归因 (属 U 冻结类, 两版本行为一致)。

### 10.1 对局档案 (pa_g1~g10, tools/games/)

| 局 | seed | 胜者 | 关键证据 |
|---|---|---|---|
| g1 | 20260990 | pypy 红 | [53] 起 ±3696 互认, 57 着吃将 |
| g2 | 20260991 | 和 | 全场无任何 ±3696, 73 着循环判和 |
| g3 | 20260992 | **pypy510 黑** | [94] 基线真杀 (pypy 自己确认 -3696 d12), 唯一败局 |
| g4 | 20260993 | pypy 黑 | [56] 起 ±3696 互认 4 回合成杀 |
| g5 | 20260994 | pypy 红 | **[46] 基线假杀王现场** (见 10.3); [92] 起真杀互认 |
| g6 | 20260995 | 和 | 基线 [123][125] 连报 +3696 却无法兑现, 138 着循环和 (见 10.4) |
| g7 | 20260996 | 和 | 全场无 ±3696, 57 着循环判和 |
| g8 | 20260997 | pypy 黑 | [74] 起 ±3696 互认, 80 着吃将 |
| g9 | 20260998 | pypy 红 | [90] 基线自报 -3696, 至 [97] 成杀 |
| g10 | 20260999 | pypy 黑 | [65] 基线自报 -3696 d12 (自认被杀), [70] 成杀 |

### 10.2 证据一: 6 胜全部为客观强制杀, 不是评分伪影

判据: 杀王序列中每一回合败方引擎自己的搜索都报 -3696 (双方引擎对同一强制线意见一致)。
- g1: [54][56] 黑 -3696 → [57] 吃将; g4: [57][59][61] 红 -3696 → [62] 吃将;
- g5: [92][94][96][98] 黑 -3696 (d11→d12) → [99] 吃将; g8: [75][77][79] → [80];
- g9: [90][92][94][96] → [97]; g10: [65][67][69] → [70]。
若胜负由随机波动造成, 败方不应在每一步都自行确认被强制杀。g10 更是基线在 pypy 动手前
就自报 -3696 (第 65 着, depth=12), 败局在双方眼中都已是客观事实。

### 10.3 证据二 (实锤): g5 第 46 着假杀王现场复现

实战 (pa_g5.log): 前 45 着局面均衡 (±110 内震荡), [46] 基线执黑走 f3f8 吃相并宣告
+3696 (depth=8); 红方应着 [47] d0d1 直接吃掉这只"杀棋主力车", 基线下一手自评立刻
回落到 -40 —— 所谓杀王一步即被推翻, 其后 50 着局面照常, 最终基线反被真杀。

回放该局面 (seed=20260994 第 46 着前, 等预算 1.5s×3 次, PyPy):

| 引擎 | 试1 | 试2 | 试3 |
|---|---|---|---|
| v5.10 基线 | f3f2 +224 | **f3f8 +3696** | **f3f8 +3696** |
| v5.11 Plan A | f3f2 +224 | f3f2 +224 | f3f2 +224 |

两模块唯一差异就是 Plan A 豁免, 故基线的 +3696 必出自 TT 杀王分零窗口回流 (正是送车局
定位的传播层 bug): 根节点在"正常着 +224"与"被染成 +3696 的假杀着"之间随迭代加深时机
摇摆, 实战恰好选中假杀着。修复版 3/3 稳定选 +224、从不宣告杀王 —— 修复精确命中该局面。

### 10.4 证据三: 杀王宣告转化率不对称 + g6 反例的诚实排除

- v5.11 的 +3696 宣告: 6 次 (g1/g4/g5/g8/g9/g10), 6/6 全部兑现为胜, 且全部被对方 -3696 逐着确认;
- 基线的 +3696 宣告: 3 次, 仅 g3 兑现 (即其唯一胜局); g5 [46] 一步即被推翻;
  g6 [123][125] 连报两次, 两着后基线自己的搜索只剩 +109 (杀线从自己手里蒸发), 终成和棋。

对 g6 第 123 着前局面同样做了对照复现: v5.10 与 v5.11 行为完全一致 (各 1/3 次 +292、
2/3 次 +3696)。说明 g6 的假宣告不经过 Plan A 修复的路径, 属 U 冻结盲点类 (已知未修局限,
双方共享), 据此把 g6 从 Plan A 的归因链中剔除 —— 该局是和棋, 不影响归因结论。

### 10.5 证据四: 统计与公平性

- 有胜负的 7 局中 pypy 赢 6: 符号检验 P(X≥6 | n=7, p=0.5) = 8/128 ≈ 6.3%。
  单看统计并不充分 (样本小), 但与 10.2~10.4 的机制证据方向完全一致: 波动无法解释
  "败方逐着自认被杀"与"同一局面修复前后行为差异"。
- 公平性: 双方均 1.13s/着、最大 1.56s 内; 基线平均深度还高 0.15 层, 算力不对称
  即使存在也是对 pypy 不利的方向。
- 唯一败局 g3: pypy 的败着是 [93] 一次普通翻开 (自评 +38) 后被基线发现真杀。
  Plan A 只改变"杀王区间分参与 TT 截断"这一条件, 不改变评估与走法生成,
  机制上不可能制造 +38 分开局层面的失误 —— 该局归入正常战术波动。

### 10.6 残留与后续

1. U 冻结盲点未修 (g6 型假宣告仍会双方同时出现), 长期方案仍是 PIMC 期望搜索;
2. 10 局样本的统计功效有限, 若需对外宣称强度, 建议补 30+ 局同配置复测;
3. 复现脚本为一次性 (tools/_tmp_diag_pa.py, 跑完已删), 方法: 按 seed 找棋谱 →
   referee.new_game/apply_move 回放到目标着前 → 双引擎各搜 3 次对照。
